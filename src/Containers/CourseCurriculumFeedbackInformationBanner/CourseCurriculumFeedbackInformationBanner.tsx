...
const CourseCurriculumFeedbackInformationBanner = () => {
    ...
    const [submitRequestError2, setSubmitRequestError2] = useState(false)
    const [submitRequestProcessing, setSubmitRequestProcessing] = useState(false)
    const [submitRequestApproved, setSubmitRequestApproved] = useState(false)
            ...
            {submitRequestProcessing && (
                <Error notShowIndicator type={'processing'} borderRadius='10px'>
                    <div className={classes.errorContainer}>
                        ...
                        <p>Great job with the curriculum 🎉. Some modules need revision. For details, select "Feedback". Please revise and resubmit for approval.</p>
                        <div className={classes.buttonContainer}>
                            <Button onClick={() => {
                                navigate('/courses/courses-curriculum/submission-guide')
                            }} type='invalid'>Read submission guide</Button>
            ...
            {submitRequestApproved && (
                <Error notShowIndicator type={'approved'} borderRadius='10px'>
                    <div className={classes.errorContainer}>
                        ...
                        <p>Course successfully approved. Well done on creating an outstanding curriculum! 🌟</p>
                    </div>
                </Error>
            )}
            {submitRequestError2 && (
                <Error notShowIndicator type={'error'} borderRadius='10px'>
                    <div className={classes.errorContainer}>
                        ...
                        <p>Editing is temporarily unavailable for courses under review. You can make edits once the review is over.</p>
                    </div>
                </Error>
            )}
        </>
